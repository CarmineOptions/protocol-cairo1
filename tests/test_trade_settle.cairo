...
#[test]
fn test_trade_settle_long_terminal_price_below() {
...
fn test_trade_settle_long_ATM() {
    ...
    assert(stats_1.unlocked_capital_c == 5003469355391826135, 'Call1 unlocked wrong');
    assert(stats_1.unlocked_capital_p == 5108644562, 'Put1 unlocked wrong');
...
#[test]
fn test_trade_settle_long_terminal_price_above() {
    let (ctx, dsps) = deploy_setup();
    let five_tokens: u256 = 5000000000000000000; // with 18 decimals
    let five_k_tokens: u256 = 5000000000; // with 6 decimals
    let one_int = 1000000000000000000; // 1*10**18
    ...
    assert(stats_1.bal_eth == 5059030644608173865, 'Eth1 bal wrong');
    assert(stats_1.bal_usdc == 4891355438, 'Usdc1 bal wrong');
    ...
    assert(stats_1.lpool_balance_c == 4940969355391826135, 'Call1 lpool bal wrong');
    assert(stats_1.lpool_balance_p == 5108644562, 'Put1 lpool bal wrong');

    assert(stats_1.bal_eth + stats_1.lpool_balance_c == 2 * five_tokens, 'random eth appeared');
    assert(stats_1.bal_usdc + stats_1.lpool_balance_p == 2 * five_k_tokens, 'random usdc appeared');

    assert(stats_1.unlocked_capital_c == 4940969355391826135, 'Call1 unlocked wrong');
    assert(stats_1.unlocked_capital_p == 5108644562, 'Put1 unlocked wrong');
...
#[test]
fn test_trade_settle_short_terminal_price_below() {
    let (ctx, dsps) = deploy_setup();
    let five_tokens: u256 = 5000000000000000000; // with 18 decimals
    let five_k_tokens: u256 = 5000000000; // with 6 decimals
    let one_int = 1000000000000000000; // 1*10**18
...
#[test]
fn test_trade_settle_short_ATM() {
    let (ctx, dsps) = deploy_setup();
    let five_tokens: u256 = 5000000000000000000; // with 18 decimals
    let five_k_tokens: u256 = 5000000000; // with 6 decimals
    let one_int = 1000000000000000000; // 1*10**18
    ...
    assert(stats_1.bal_eth == 5001506561362561699, 'Eth1 bal wrong');
    assert(stats_1.bal_usdc == 5098635179, 'Usdc1 bal wrong');
...
#[test]
fn test_trade_settle_short_terminal_price_above() {
    let (ctx, dsps) = deploy_setup();
    let five_tokens: u256 = 5000000000000000000; // with 18 decimals
    let five_k_tokens: u256 = 5000000000; // with 6 decimals
    let one_int = 1000000000000000000; // 1*10**18
    ...
    assert(stats_1.bal_eth == 4939006561362561699, 'Eth1 bal wrong');
    assert(stats_1.bal_usdc == 5098635179, 'Usdc1 bal wrong');

    assert(stats_1.lpool_balance_c == 5060993438637438301, 'Call1 lpool bal wrong');
    assert(stats_1.lpool_balance_p == 4901364821, 'Put1 lpool bal wrong');

    assert(stats_1.bal_eth + stats_1.lpool_balance_c == 2 * five_tokens, 'random eth appeared');
    assert(stats_1.bal_usdc + stats_1.lpool_balance_p == 2 * five_k_tokens, 'random usdc appeared');

    assert(stats_1.unlocked_capital_c == 5060993438637438301, 'Call1 unlocked wrong');
    assert(stats_1.unlocked_capital_p == 4901364821, 'Put1 unlocked wrong');
...
#[test]
#[should_panic(expected: ('Option not expired',))]
fn test_trade_settle_pool_not_ripe() {
    let (ctx, dsps) = deploy_setup();
    let five_tokens: u256 = 5000000000000000000; // with 18 decimals
    let five_k_tokens: u256 = 5000000000; // with 6 decimals
    let one_int = 1000000000000000000; // 1*10**18
    ...
    // Try to expire pool
    // We have to expire short here since pool has no long pos 
    // so it does nothing and doesn't fail
    dsps
        .amm
        .expire_option_token_for_pool(
...
#[test]
#[should_panic(expected: ('Settle - option not expired',))]
fn test_trade_settle_not_ripe() {
    let (ctx, dsps) = deploy_setup();
    let five_tokens: u256 = 5000000000000000000; // with 18 decimals
    let five_k_tokens: u256 = 5000000000; // with 6 decimals
    let one_int = 1000000000000000000; // 1*10**18
