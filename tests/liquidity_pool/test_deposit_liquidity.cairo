...
fn test_deposit_liquidity() {
    ...
    let long_call_premia = dsps.amm.trade_open(
        0,  // Call
        ctx.strike_price, 
        ctx.expiry,
        0, // Long
    ...
    let short_call_premia = dsps.amm.trade_open(
        0,  // Call
        ctx.strike_price, 
        ctx.expiry,
        1, // Short
    ...
    let long_put_premia = dsps.amm.trade_open(
        1,  // Put
        ctx.strike_price, 
        ctx.expiry,
        0, // Long
    ...
    let short_put_premia = dsps.amm.trade_open(
        1,  // Put
        ctx.strike_price, 
        ctx.expiry,
        1, // Short
    ...
    assert(short_call_premia == FixedTrait::from_felt(14425957268832963), 'Short Call premia wrong');

    assert(long_put_premia == FixedTrait::from_felt(1869892212451346729800), 'Long put premia wrong');
    assert(short_put_premia == FixedTrait::from_felt(1869892212451346729800), 'Short put premia wrong');
    ...
    assert(stats_2.bal_eth == 2999953078037366845, 'Eth2 bal wrong'); // 4 - 1 - long_prem_w_fees + short_prem_w_fees
    assert(stats_2.bal_usdc == 2493917977, 'Usdc2 bal wrong'); // 4k - 1.5 - long_prem_w_fees + short_prem_w_fees
